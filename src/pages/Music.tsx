...
const Music = () => {
  ...
  const fetchMusic = async () => {
    try {
      const { data, error } = await supabase
        .from("content")
        .select("*")
        .in("content_type", ["music", "music_video"])
  ...
  const deleteContent = async (id: string, fileUrl?: string) => {
    try {
      ...
      setShowDetailModal(false);
      toast({ title: "Content deleted", description: "Music video removed." });
    } catch (err: any) {
  ...
  const hipHop = content.filter((c) => c.genre?.toLowerCase().includes("hip"));
  const rnb = content.filter((c) => c.genre?.toLowerCase().includes("r&b") || c.genre?.toLowerCase().includes("rnb"));
  const pop = content.filter((c) => c.genre?.toLowerCase().includes("pop"));
  const myMusic = user ? content.filter((c) => c.user_id === user.id) : [];
      ...
      <div className="pb-20 pt-20 relative z-10">
        ...
        {content.length === 0 && (
          <div className="text-center py-20">
            <h2 className="text-2xl font-bold text-foreground mb-4">No Music Yet</h2>
            <p className="text-muted-foreground">Upload your first track or music video.</p>
          </div>
        )}
      </div>
