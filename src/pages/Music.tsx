...
const REST_TABLE = "contents"; // change if your table/view has a different name
...
export default function Music() {
  ...
  const fetchPage = async (start: number, size: number) => {
    if (!API_URL || !API_KEY) {
      console.warn("[Music] Missing VITE_SUPABASE_URL or VITE_SUPABASE_PUBLISHABLE_KEY");
      return [];
    }
    const params = new URLSearchParams({
      ...
      limit: String(size),
      offset: String(start),
      content_type: "in.(music)",
    });
    const res = await fetch(`${API_URL}/rest/v1/${REST_TABLE}?${params.toString()}`, {
      headers: { apikey: API_KEY, Authorization: `Bearer ${API_KEY}` },
    });
    if (!res.ok) {
      console.error("[Music] REST error", res.status, await res.text());
      return [];
    }
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 text-white">
      <h1 className="text-2xl sm:text-3xl font-extrabold">Music</h1>
      <p className="mt-2 text-gray-400">Discover teen-created tracks and videos.</p>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((it, i) => (
          <ContentCard
            ...
            contentType="Music"
            index={i}
            onClick={() => navigate(`/music/${it.id}`)}
            onPlay={() => navigate(`/music/${it.id}`)}
          />
        ))}
      </div>

      {items.length === 0 && !loading && (
        <div className="mt-6 text-sm text-gray-400">No music yet.</div>
      )}

      {hasMore && (
        <div className="mt-8">
          <Button onClick={() => load(true)} disabled={loading} className="bg-pink-500 hover:bg-pink-600">
