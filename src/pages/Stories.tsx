...
export default function Stories() {
  ...
  const fetchPage = async (start: number, size: number) => {
    if (!API_URL || !API_KEY) {
      console.warn("[Stories] Missing VITE_SUPABASE_URL or VITE_SUPABASE_PUBLISHABLE_KEY");
      return [];
    }
    const params = new URLSearchParams({
      ...
      limit: String(size),
      offset: String(start),
      content_type: "in.(story)",
    });
    const res = await fetch(`${API_URL}/rest/v1/${REST_TABLE}?${params.toString()}`, {
      headers: { apikey: API_KEY, Authorization: `Bearer ${API_KEY}` },
    });
    if (!res.ok) {
      console.error("[Stories] REST error", res.status, await res.text());
      return [];
    }
