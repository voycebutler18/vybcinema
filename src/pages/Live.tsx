...
export default function Live() {
  ...
  const fetchPage = async (start: number, size: number) => {
    if (!API_URL || !API_KEY) {
      console.warn("[Live] Missing VITE_SUPABASE_URL or VITE_SUPABASE_PUBLISHABLE_KEY");
      return [];
    }
    const params = new URLSearchParams({
      ...
      limit: String(size),
      offset: String(start),
      content_type: "in.(live)",
    });
    const res = await fetch(`${API_URL}/rest/v1/${REST_TABLE}?${params.toString()}`, {
      headers: { apikey: API_KEY, Authorization: `Bearer ${API_KEY}` },
    });
    if (!res.ok) {
      console.error("[Live] REST error", res.status, await res.text());
      return [];
    }
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 text-white">
      <h1 className="text-2xl sm:text-3xl font-extrabold">Live</h1>
      <p className="mt-2 text-gray-400">Watch live events and streams.</p>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((it, i) => (
          <ContentCard
            ...
            onPlay={() => navigate(`/live/${it.id}`)}
          />
        ))}
      </div>

      {items.length === 0 && !loading && (
        <div className="mt-6 text-sm text-gray-400">No live events yet.</div>
      )}
