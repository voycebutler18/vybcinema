...
const Music = () => {
  ...
  const rnb = content.filter((v) =>
    v.genre?.toLowerCase().includes("r&b") || v.genre?.toLowerCase().includes("soul")
  );
  const rock = content.filter((v) => v.genre?.toLowerCase().includes("rock"));
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-6 pt-28 pb-16">
          <h2 className="text-2xl md:text-3xl font-bold mb-6">Latest Music</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        {!content.length && (
          <div className="container mx-auto px-6">
            <div className="bg-card/60 border border-border/40 rounded-2xl p-8 text-center">
              <h2 className="text-2xl font-bold">No Music Videos Yet</h2>
              <p className="text-muted-foreground">Upload your first music video!</p>
            </div>
          </div>
        )}
      </main>
