...
const Talent = () => {
  ...
  const deleteContent = async (id: string, fileUrl?: string) => {
    try {
      ...
      setShowDetailModal(false);
      toast({ title: "Deleted", description: "Talent video removed." });
    } catch (err: any) {
  ...
  const singing = content.filter((v) => v.genre?.toLowerCase().includes("sing"));
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-6 pt-28 pb-16">
          <h2 className="text-2xl md:text-3xl font-bold mb-6">Recently Added Talent</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-20">
        <Section title="Recently Added Talent" items={recentlyAdded} />
        ...
        {!content.length && (
          <div className="container mx-auto px-6">
            <div className="bg-card/60 border border-border/40 rounded-2xl p-8 text-center">
              ...
              <p className="text-muted-foreground">Show off your skills—upload your first video!</p>
            </div>
          </div>
        )}
      </main>
