...
const REST_TABLE = "contents"; // change if needed
...
export default function Shows() {
  ...
  const fetchPage = async (start: number, size: number) => {
    if (!API_URL || !API_KEY) {
      console.warn("[Shows] Missing VITE_SUPABASE_URL or VITE_SUPABASE_PUBLISHABLE_KEY");
      return [];
    }
    const types = ["show", "tv", "tv-show"];
    const params = new URLSearchParams({
      select: "id,title,description,genre,cover_url,file_url,trailer_url,preview_url,content_type,created_at",
      order: "created_at.desc",
      limit: String(size),
      offset: String(start),
      content_type: `in.(${types.join(",")})`,
    });
    const res = await fetch(`${API_URL}/rest/v1/${REST_TABLE}?${params.toString()}`, {
      headers: { apikey: API_KEY, Authorization: `Bearer ${API_KEY}` },
    });
    if (!res.ok) {
      console.error("[Shows] REST error", res.status, await res.text());
      return [];
    }
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 text-white">
      <h1 className="text-2xl sm:text-3xl font-extrabold">Shows</h1>
      <p className="mt-2 text-gray-400">Original teen shows and web series.</p>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((it, i) => (
          <ContentCard
            ...
            contentType="Shows"
            index={i}
            onClick={() => navigate(`/shows/${it.id}`)}
            onPlay={() => navigate(`/shows/${it.id}`)}
          />
        ))}
      </div>

      {items.length === 0 && !loading && (
        <div className="mt-6 text-sm text-gray-400">No shows yet.</div>
      )}
